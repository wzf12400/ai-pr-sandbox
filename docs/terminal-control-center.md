...
arguments it shows a compact 256-color bovine pixel mascot and the input prompt;
non-color output falls back to a Unicode silhouette. The function menu appears
only after `help`. Employees may enter a natural-language change request,
`logs`/`日志`/`/logs`, `inbox`/`收件箱`,
...
creating a second item. Re-reading the exact same incident reference is an
overlap duplicate and does not increase the historical event count. A new
incident reference with the same service, normalized endpoint, and exception
adds its actual event count to the existing record.

The selector collapses matching endpoint/error candidates into one row. The
inbox and generated Issue show first observed, last observed, current-scan log
events, historical matching log events, affected endpoints, matching incident
groups, and affected-user bounds with identifier coverage. These fields are
derived locally rather than generated by the model. User IDs and HMAC user
references are not stored in the inbox or Issue; when user evidence is absent,
the affected-user field remains `unknown`.
